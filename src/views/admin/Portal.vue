...
const submitData = () => {
  loading.value = true;
  try {
    Api.post(`admin/add-user-label`, data.value, {
      headers: {
...
watch(selectedUser, (val) => {
  if (val) {
    data.value.selectedUserId = val.id;
  } else {
    data.value.selectedUserId = "";
  }
});
...
<template>
  <div class="py-5 bg-light">
    <div class="container">
      <div class="text-end">
        <button
          class="btn btn-primary text-white text-decoration-none mb-2 font-medium"
          @click="logOut"
        >
          Logout
        </button>
      ...
      <div class="row justify-content-center">
        <div class="col-12">
          <div class="card mb-4">
            ...
            <div class="card-body">
              <label class="form-label fs-5">Select a user to update</label>
              <select
                id="selectedUserId"
                class="form-select"
              ...
              >
                ...
                <option v-if="allUsers.length === 0" disabled>
                  No users found
                </option>
              </select>
              <small
                class="text-danger"
                v-for="message of validation_message.selectedUserId"
                :key="`${message}-selectedUserId`"
                >{{ message }}</small
              >
              <div class="mt-3" v-if="selectedUser">
                <h5 class="mb-2">Name: {{ selectedUser.name }}</h5>
                <h5>Available Balance: ${{ selectedUser.balance }}</h5>
              </div>
            </div>
          </div>
          <div class="card mb-4">
            <div class="card-header bg-secondary">
              <h4 class="text-primary mb-0">Label Details</h4>
            </div>
            <div class="card-body">
              <div class="row">
                <div class="col-md-6 mb-3">
                  ...
                  <small
                    class="text-danger"
                    v-for="message of validation_message.price"
                    :key="`${message}-price`"
                    >{{ message }}</small
                  >
                </div>
                <div class="col-md-6 mb-3">
                  ...
                  <small
                    class="text-danger"
                    v-for="message of validation_message.tracking_number"
                    :key="`${message}-tracking_number`"
                    >{{ message }}</small
                  >
                </div>
                <div class="col-md-6 mb-3">
                ...
                <div class="col-md-6 mb-3">
                  ...
                  <small
                    class="text-danger"
                    v-for="message of validation_message.length"
                    :key="`${message}-length`"
                    >{{ message }}</small
                  >
                </div>
                <div class="col-md-6 mb-3">
                  ...
                  <small
                    class="text-danger"
                    v-for="message of validation_message.width"
                    :key="`${message}-width`"
                    >{{ message }}</small
                  >
                </div>
                <div class="col-md-6 mb-3">
                  ...
                  <small
                    class="text-danger"
                    v-for="message of validation_message.height"
                    :key="`${message}-height`"
                    >{{ message }}</small
                  >
                </div>
                <!-- <div class="col-md-6 mb-3">
                  <label for="file" class="form-label">Upload Label File</label>
                  <input
                    class="form-control"
  ...
  <div class="payment_toast position-fixed">
    ...
    >
      <div class="d-flex">
        <div class="toast-body">Label generated successfully!</div>
        <button
          type="button"
    ...
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        ...
        <div class="modal-body text-center py-4">
          <p class="fs-4 mb-1 font-light">User have insufficient balance.</p>
        </div>
      </div>
    </div>
  </div>
  ...
  >
    <div class="text-center">
      ...
      <h5>
        Please wait! <br />
        <span>This may take some time</span> <br />
